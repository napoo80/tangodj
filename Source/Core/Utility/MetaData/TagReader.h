...
public:
    ...
    /** Reads metadata tags to fill Media information in. Returns Media with all of the relevant information about the file     
     @param audioFile The file to be parsed
     @return Media containing all of the information about the file
     */
	static Song parseSong (const File& audioFile)
	{
        Song song;
		if (audioFile.existsAsFile())
        {
			...
			if (reader != nullptr)
            {
                ...
                #if JUCE_MAC
                TagLib::FileName theFile (audioFile.getFullPathName().toUTF8 ());
                #else
                TagLib::FileName theFile (audioFile.getFullPathName().toWideCharPointer ());
                #endif
				TagLib::FileRef f(theFile);
				song.artist =  String (CharPointer_UTF8(f.tag()->artist().toCString(true)));
                
                //If no tag song title exists, set the song title to file name
                if (f.tag()->title().isEmpty())
                    song.name = audioFile.getFileName();
                else
                    song.name = String (CharPointer_UTF8(f.tag()->title().toCString(true)));
                
                if(song.name.startsWith("···")){
                    song.name = song.name.removeCharacters("·");
                    song.isCortina = true;
                }
                
                song.gender = f.tag()->genre().toCString();
                song.comment = String (CharPointer_UTF8(f.tag()->comment().toCString(true)));
                song.year = f.tag()->year();
                song.filePath = audioFile.getFullPathName();
                song.sampleRate = reader->sampleRate;
             
                song.length = ((reader->lengthInSamples/reader->sampleRate));
                return song;
